...
class Sample:
    ...
    def check_span(self, a_matrix: np.ndarray, y_vector: np.ndarray) -> bool:
        ...
        """
        If rank of A is not full, then y is not in the span of columns of A
        It means if there exist a solution for Ax = y, then y is in the span of columns of A.
        If rank(A) = rank([A|y]), then y is in the column span of A.
        """
        augmented_matrix = np.hstack((a_matrix, y_vector.reshape(-1,1)))

        if np.linalg.matrix_rank(a_matrix) == np.linalg.matrix_rank(augmented_matrix):
            return True
        else:
            return False
